...
import { userAPI, subjectAPI, attendanceAPI } from "../../api/apiService";
...
function ViewAttendance() {
  ...
  const fetchFilteredAttendance = () => {
    if (role === "admin") {
      ...
      attendanceAPI
        .getAttendance(selectedFaculty, selectedSubject, selectedDate)
        .then((res) => {
          const data = res.data;
          setAttendance(Array.isArray(data) ? data : []);
        })
      ...
      attendanceAPI
        .getAttendance(username, selectedSubject, selectedDate)
        .then((res) => {
          const data = res.data;
          setAttendance(Array.isArray(data) ? data : []);
        })
          ...
          >
            ...
            >
              ...
              <div className="flex flex-wrap gap-2">
                {students.length > 0 ? (
                  students.map((s) => (
                    <span key={s.id} className="px-3 py-1 bg-gray-100 rounded">
                      {s.name}
                    </span>
                  ))
                ) : (
